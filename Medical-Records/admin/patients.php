							...
							<tbody class="text-center">
							   ...
							   ?>
								<tr class="active">
									...
									<td>
										<a style="background: #FFF; color:#333" href="patients.php?do=Edit&patientid=<?php  echo $pat["national_id"]; ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i> تعديل</a>
										<a onclick="return confirm('هل أنت متأكد؟');" style="background: #FFF; color:#333" href="patients.php?do=Delete&patientid=<?php  echo $pat["national_id"]; ?>" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>
								<?php } ?>
							</tbody>
		...
		if($_SERVER['REQUEST_METHOD'] =='POST'){
		 ...
		 $count = $stmt->rowCount();
			...
			if($count > 0){
				...
				$stmt = $con->prepare("DELETE FROM medical_record WHERE national_id = :patientid");

				$stmt->bindParam(":patientid" , $patientid);

				$stmt->execute();

				echo '<div style="margin-top:40px" dir="rtl" class="alert alert-info">تم حذف هذا المريض بنجاح
				<button type="button" class="close pull-left" data-dismiss="alert" aria-label="Close">
						<span aria-hidden="true">&times;</span>
						</button>
				</div>';
						...
						<table class="table" dir="rtl">
							...
							<tbody class="text-center">';
								...
								echo '<tr class="active">
									...
									<td>' . $pat["Email"] . '</td>
									<td>
										<a style="background: #FFF; color:#333" href="patients.php?do=Edit&patientid=' . $pat["national_id"] . '" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i> تعديل</a>
										<a onclick="return confirm(\'هل أنت متأكد؟\');" style="background: #FFF; color:#333" href="patients.php?do=Delete&patientid=' . $pat["national_id"] . '" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>';
								}
							echo '</tbody>
				...
				echo "<div class='alert alert-danger'>هذا المريض غير مسجل عندك</div>";
