		...
		<div id="page-wrapper" class="pull-left" style="height:800px">
					...
					<div class="bs-example widget-shadow" data-example-id="contextual-table"> 
						...
						<table class="table" dir="rtl">
							...
							<tbody class="text-center">
								...
								include('../connect.php');  
								$sql = $con->prepare("SELECT * FROM admin WHERE acceptance=1");
								$sql->execute();
								$rows = $sql->fetchAll();
							   ...
							   ?>
								<tr class="">
									...
									<td>
										<a style="background: #FFF; color:#333" href="schools.php?do=Delete&schoolid=<?php echo $pat["admin_ID"]; ?>" class="btn btn-danger confirm"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>
								<?php  } ?>
							</tbody>
		 ...
		 $count = $stmt->rowCount();
			...
			if($count > 0){
				...
				$stmt = $con->prepare("DELETE FROM admin WHERE admin_ID = :schoolid");

				$stmt->bindParam(":schoolid" , $schoolid);

				$stmt->execute();

				echo '<div style="margin-top:40px" dir="rtl" class="alert alert-info">تم حذف هذه المدرسة بنجاح
						...
						<table class="table" dir="rtl">
							...
							<tbody class="text-center">';
								...
								echo '<tr class="">
									...
									<td>' . $pat["Stage"] . '</td>
									<td>
										<a style="background: #FFF; color:#333" href="schools.php?do=Delete&schoolid=' . $pat["admin_ID"] . '" class="btn btn-danger confirm"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>';
							 } 
							echo '</tbody>
