		...
		<div id="page-wrapper" class="pull-left" style="height:800px">
			<div class="main-page">
				...
				<div class="tables" style="margin-top:40px;height:800px">
					...
					<h3 style="display:inline" class="title1">المعلمين</h3>
					<div class="bs-example widget-shadow" data-example-id="contextual-table"> 
						<h4>ادارة المعلمين :</h4>
							...
							<tbody class="text-center">
							   ...
							   ?>
								<tr>
									...
									<td>
										<a onclick="return confirm('هل أنت متأكد؟');" style="background: #FFF; color:#333;width:170px" href="teachers.php?do=Delete&teacherid=<?php echo $pat["teacher_ID"]; ?>" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>
								<?php  } ?>
							</tbody>
		 ...
		 include('../connect.php');  						  
		 $stmt = $con->prepare("SELECT * FROM teachers WHERE teacher_ID= ?");
		 ...
		 $count = $stmt->rowCount();
			...
			if($count > 0){
				...
				$stmt = $con->prepare("DELETE FROM teachers WHERE teacher_ID = :teacherid");

				$stmt->bindParam(":teacherid" , $teacherid);

				$stmt->execute();

				echo '<div style="margin-top:40px" dir="rtl" class="alert alert-info">تم حذف هذا المعلم بنجاح
					...
					<div class="bs-example widget-shadow" data-example-id="contextual-table"> 
						...
						<table class="table" dir="rtl">
							...
							<tbody class="text-center">';
								...
								echo '<tr>
									<th scope="row">' . $pat["teacher_ID"] . '</th>
									<td>' . $pat["Fname"] . ' ' . $pat["Lname"] . '</td>
									<td>' . $pat["Email"] . '</td>
									<td>0' . $pat["Phone"] . '</td>
									<td>
										<a onclick="return confirm(\'هل أنت متأكد؟\');" style="background: #FFF; color:#333;width:170px" href="teachers.php?do=Delete&teacherid=' . $pat["teacher_ID"] . '" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>';
							   }
							echo '</tbody>
				...
				echo "<div class='alert alert-danger'>هذا المعلم غير مسجل عندك</div>";
