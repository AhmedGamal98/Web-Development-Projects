		...
		<div id="page-wrapper" class="pull-left" style="height:800px">
			<div class="main-page">
				...
				<div class="tables" style="margin-top:40px;height:700px">
					...
					<div class="bs-example widget-shadow" data-example-id="contextual-table"> 
						<h4>ادارة الاعلانات والتنبيهات:</h4>
							...
							<tbody class="text-center">
							   ...
							   ?>
								<tr>
									...
									<td>
										<a onclick="return confirm('هل أنت متأكد؟');" style="background: #FFF; color:#333;width:170px" href="messages.php?do=Delete&messageid=<?php echo $pat["message_ID"]; ?>" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>
								<?php } ?>
							</tbody>
				...
				<div class="widget-shadow">
					<div class="login-body">
						<form action="?do=Insert" method="post">
							...
							<textarea name="message" class="form-control" placeholder="الاعلان أو التنبيه" style="resize:none;height:200px;margin-bottom:20px"></textarea>
			...
			if(empty($formErrors)){
				...
				//$check = checkItem("Username" , "users" , $username);
				 ...
				 include('../connect.php');  						  
				 $stmt = $con->prepare("SELECT * FROM messages WHERE message= ?");
				...
				<div class="widget-shadow">
					...
					$stmt = $con->prepare("INSERT INTO messages(message, admin_ID) VALUES(:message,  :adminid)");
		 ...
		 include('../connect.php');  						  
		 $stmt = $con->prepare("SELECT * FROM messages WHERE message_ID= ?");
		 ...
		 $count = $stmt->rowCount();
			...
			if($count > 0){
				...
				$stmt = $con->prepare("DELETE FROM messages WHERE message_ID = :messageid");

				$stmt->bindParam(":messageid" , $messageid);

				$stmt->execute();

				echo '<div style="margin-top:40px" dir="rtl" class="alert alert-info">تم حذف هذا الاعلان أو التنبيه بنجاح
				<button type="button" class="close pull-left" data-dismiss="alert" aria-label="Close">
						<span aria-hidden="true">&times;</span>
						</button>
				</div>';
					...
					<div class="bs-example widget-shadow" data-example-id="contextual-table"> 
						...
						<table class="table" dir="rtl">
							...
							<tbody class="text-center">';
								...
								echo '<tr>
									<th scope="row">' . $pat["message_ID"] . '</th>
									<td>' . $pat["message"] . '</td>
									<td>
										<a onclick="return confirm(\'هل أنت متأكد؟\');" style="background: #FFF; color:#333;width:170px" href="messages.php?do=Delete&messageid=' .  $pat["message_ID"] . '" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i> حذف</a>
									</td>
								</tr>';
								}
							echo '</tbody>
				...
				echo "<div class='alert alert-danger'>هذا الاعلان غير مسجل عندك</div>";
