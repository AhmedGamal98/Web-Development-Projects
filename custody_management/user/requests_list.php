...
<html lang="en">
    ...
    <head>
        ...
        <title>Custody Management - Requests</title>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table class="table datatable">
                                    ...
                                    <tbody>
                                    <?php
                                          $sql =$con->prepare("SELECT * FROM requests where to_user_id=?");
                                          $sql->execute(array($_SESSION['id']));
                                          
                                          $rows =$sql->fetchAll($con::FETCH_ASSOC);
                                            foreach ($rows as $row){
                                            $sql1 =$con->prepare("SELECT * FROM users where user_id=?");
                                            $sql1->execute(array($row['from_user_id']));                                           
                                            $row1 =$sql1->fetch();
                                            ...
                                            $row2 =$sql2->fetch();
                                              if($row['status'] == 0){
                                                ...
                                                <td><a onclick="return confirm(\'Are you sure you want to accept this request?\')"
                                                        href="../include/accept_request.php?id='.$row['request_id'].'"><button type="button"
                                                            class="btn btn-outline-success">Accept</button></a>
                                                    <a href="../include/reject_request.php?id='.$row['request_id'].'"
                                                        onclick="return confirm(\'Are you sure you want to reject this request?\')"><button
                                                            type="button" class="btn btn-outline-danger">Reject</button></a>
                                                </td>
