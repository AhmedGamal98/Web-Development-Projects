    ...
    <nav class="navbar sticky-top navbar-expand-lg navbar-light bg-light" style="padding-top: 20px;">
      ...
      <header>
        <div class="content" style="margin-top: 250px;">
          <div class="container">
            <h1 style="text-align: center; color:#fff; font-size:60px;">طلبات الاستشارة</h1><br>
            <table class="table">
			...
			<?php
				...
				include('connection.php');  
				$sql = $con->prepare("SELECT * FROM requests");
				$sql->execute();
				$rows = $sql->fetchAll();
			  ...
			  ?>	
                <tr>
                  ...
                  <td><?php if($pat["state"] == 0){echo "طلب جديد";}else{echo "تم الرد";} ?></td>
                  <td><?php echo $pat["title"]; ?></td>
                  <td><?php echo $pat["date"]; ?></td>
				 <?php if($pat["state"] == 0){ ?>	
                  <td><a href="response.php?requestid=<?php echo $pat["request_id"]; ?>"><button class="btn btn-outline-primary active" id="s1_btn" type="button">رد</button></a> </td>
